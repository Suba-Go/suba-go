...
export default function CompanySettingsForm({ initialData }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus({ type: 'error', message: error?.message || 'Ocurrió un error' });
    }
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      {!isAllowed && (
        <div className="p-3 rounded bg-yellow-50 text-yellow-800 text-sm">
          No tienes permisos para editar la configuración. Solo ADMIN o AUCTION_MANAGER.
        </div>
      )}
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <label className="block md:col-span-2">
            <span className="text-sm font-medium text-gray-700">Nombre de la Empresa</span>
            <input
              type="text"
              value={form.name}
          ...
          <label className="block md:col-span-2">
            <span className="text-sm font-medium text-gray-700">Subtítulo / Misión</span>
            <textarea
              value={form.subtitle}
              ...
              rows={3}
            />
            <span className="text-xs text-gray-500 mt-1">Este texto aparecerá debajo del nombre de tu empresa</span>
          </label>
          ...
          <label className="flex items-center gap-2 md:col-span-2">
            <input
              ...
              onChange={(e) => updateField('background_logo_enabled', e.target.checked)}
              className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
              disabled={!isAllowed}
            />
            <span className="text-sm font-medium text-gray-700">Mostrar logo como fondo transparente</span>
          </label>
      ...
      {/* Colors Section */}
      <div className="space-y-4">
        <h3 className="text-lg font-semibold text-gray-900">Colores de la Marca</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <label className="block">
            <span className="text-sm font-medium text-gray-700">Color Principal 2</span>
            <div className="mt-1 flex gap-2">
              <input
                type="color"
                value={form.principal_color2}
                onChange={(e) => updateField('principal_color2', e.target.value)}
                className="h-10 w-16 border border-gray-300 rounded cursor-pointer"
                disabled={!isAllowed}
              />
              <input
          ...
          <label className="block">
            <span className="text-sm font-medium text-gray-700">Color Secundario 2</span>
            <div className="mt-1 flex gap-2">
              <input
                type="color"
                value={form.secondary_color2}
                onChange={(e) => updateField('secondary_color2', e.target.value)}
                className="h-10 w-16 border border-gray-300 rounded cursor-pointer"
                disabled={!isAllowed}
              />
              <input
          ...
          <label className="block">
            <span className="text-sm font-medium text-gray-700">Color Secundario 3</span>
            <div className="mt-1 flex gap-2">
              <input
                type="color"
                value={form.secondary_color3}
                onChange={(e) => updateField('secondary_color3', e.target.value)}
                className="h-10 w-16 border border-gray-300 rounded cursor-pointer"
                disabled={!isAllowed}
              />
              <input
