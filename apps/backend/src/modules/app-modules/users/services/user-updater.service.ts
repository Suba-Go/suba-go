...
export class UserUpdaterService {
  ...
  async updateUserProfile(
    ...
    if (updateData.email && updateData.email !== existingUser.email) {
      const userWithEmail = await this.userPrismaRepository.findByEmail(updateData.email);
    ...
    if (updateData.rut && updateData.rut !== existingUser.rut) {
      const userWithRut = await this.userPrismaRepository.findByRut(updateData.rut);
    ...
    const updatedUser = await this.userPrismaRepository.update(userId, updateData);
